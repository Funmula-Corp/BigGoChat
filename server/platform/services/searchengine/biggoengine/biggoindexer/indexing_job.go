...
func (worker *BiggoIndexerWorker) DoJob(job *model.Job) {
	...
	if !worker.engine.IsActive() {
		appError := model.NewAppError("BiggoIndexerWorker", "biggoengine.indexer.do_job.engine_inactive", nil, "", http.StatusInternalServerError)
	...
	if startString, ok := job.Data["start_time"]; ok {
		...
		if err != nil {
			...
			appError := model.NewAppError("BiggoIndexerWorker", "biggoengine.indexer.do_job.parse_start_time.error", nil, "", http.StatusInternalServerError).Wrap(err)
			if err := worker.jobServer.SetJobError(job, appError); err != nil {
		...
		if err != nil {
			...
			appError := model.NewAppError("BiggoIndexerWorker", "biggoengine.indexer.do_job.get_oldest_entity.error", nil, "", http.StatusInternalServerError).Wrap(err)
			if err := worker.jobServer.SetJobError(job, appError); err != nil {
				logger.Error("Worker: Failed to set job error", mlog.Err(err), mlog.NamedErr("set_error", appError))
			}
			return
	...
	if endString, ok := job.Data["end_time"]; ok {
		...
		if err != nil {
			...
			appError := model.NewAppError("BiggoIndexerWorker", "biggoengine.indexer.do_job.parse_end_time.error", nil, "", http.StatusInternalServerError).Wrap(err)
			if err := worker.jobServer.SetJobError(job, appError); err != nil {
...
func (worker *BiggoIndexerWorker) IndexPostsBatch(logger mlog.LoggerIFace, progress IndexingProgress) (IndexingProgress, *model.AppError) {
	var posts []*model.PostForIndexing

	tries := 0
	for posts == nil {
		var err error
		posts, err = worker.jobServer.Store.Post().GetPostsBatchForIndexing(progress.LastEntityTime, progress.LastPostID, postBatchSize)
		if err != nil {
			if tries >= 10 {
	...
	if err := worker.engine.IndexPostsBulk(posts); err != nil {
		return progress, err
	}
...
func (worker *BiggoIndexerWorker) IndexFilesBatch(logger mlog.LoggerIFace, progress IndexingProgress) (IndexingProgress, *model.AppError) {
	var files []*model.FileForIndexing

	tries := 0
	for files == nil {
		var err error
		files, err = worker.jobServer.Store.FileInfo().GetFilesBatchForIndexing(progress.LastEntityTime, progress.LastFileID, true, fileBatchSize)
		if err != nil {
			if tries >= 10 {
	...
	if err := worker.engine.IndexFilesBulk(files); err != nil {
		return progress, err
	}
...
func (worker *BiggoIndexerWorker) IndexChannelsBatch(logger mlog.LoggerIFace, progress IndexingProgress) (IndexingProgress, *model.AppError) {
	var channels []*model.Channel

	tries := 0
	for channels == nil {
		var nErr error
		channels, nErr = worker.jobServer.Store.Channel().GetChannelsBatchForIndexing(progress.LastEntityTime, progress.LastChannelID, channelBatchSize)
		if nErr != nil {
			if tries >= 10 {
				return progress, model.NewAppError("BiggoIndexerWorker.IndexChannelsBatch", "app.channel.get_channels_batch_for_indexing.get.app_error", nil, "", http.StatusInternalServerError).Wrap(nErr)
			}
	...
	if err := worker.engine.IndexChannelsBulk(channels); err != nil {
		return progress, err
	}
...
func (worker *BiggoIndexerWorker) IndexUsersBatch(logger mlog.LoggerIFace, progress IndexingProgress) (IndexingProgress, *model.AppError) {
	var users []*model.UserForIndexing

	tries := 0
	for users == nil {
		if usersBatch, err := worker.jobServer.Store.User().GetUsersBatchForIndexing(progress.LastEntityTime, progress.LastUserID, userBatchSize); err != nil {
			if tries >= 10 {
	...
	if err := worker.engine.IndexUsersBulk(users); err != nil {
		return progress, err
	}
