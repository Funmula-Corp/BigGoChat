...
build-docker:
	cd server/build && \
	cp -r ../dist packages && \
	docker build -t docker.dev.cloud.biggo.com/test/mattermost:${DOCKER_TAG} .
...
run-docker:
	docker run -it --rm -v ./server/config/config.json:/mattermost/config/config.json docker.dev.cloud.biggo.com/test/mattermost:${DOCKER_TAG}

build-full: build-server build-package build-docker
	docker push docker.dev.cloud.biggo.com/test/mattermost:${DOCKER_TAG}
